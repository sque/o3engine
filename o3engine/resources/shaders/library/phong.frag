...
vec4 phong_lit(void);
vec4 phong_for_all_lights(vec4 mat_emission, vec4 mat_ambient, vec4 mat_diffuse, vec4 mat_specular, float mat_shininess);
...
vec4 phong_for_light(light_t light, vec3 N, vec3 L, vec3 E, float att_factor, vec4 mat_emission, vec4 mat_ambient, vec4 mat_diffuse, vec4 mat_specular, float mat_shininess) {
...
vec4 phong_for_all_lights(vec4 mat_emission, vec4 mat_ambient, vec4 mat_diffuse, vec4 mat_specular, float mat_shininess) {
	...
	for(uint i = 0u;i < TotalLights;i++) {
		...
		color += phong_for_light(Lights[0], N, L, E, 1, mat_emission, mat_ambient, mat_diffuse, mat_specular, mat_shininess);
	}
	return color;
}
...
vec4 phong_lit(void) {
	...
	for(uint i = 0u;i < TotalLights;i++) {
		vec3 L = normalize(vtLightDirWs[i]);
		color += phong_for_light(Lights[0], N, L, E, 1, vec4(0,0,0,1), vec4(0.2, 0.2, 0.2, 1), vec4(1, 1, 1, 1), vec4(1, 1, 1, 1), 50);
	}
	return color;
}
