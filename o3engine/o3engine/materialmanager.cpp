...
namespace o3engine
{
	...
	template<> MaterialManager* SingletonManager<MaterialManager, string, Material>::ms_singleton = 0;

	// Constructor
	MaterialManager::MaterialManager()
	{
		pDefaultMaterial = new Material("default");
	}
	...
	// Parse a materials file and load everythin
	...
	{   FILE * fin;
		...
		while(!line_str.empty())
		{
			...
			{
				tmpColor = _parse_color4(tmp_tokens[1], tmp_tokens[2], tmp_tokens[3], tmp_tokens[4]);
			...
			{
				if (tmp_tokens[1] == "off") pMat->disableDepthWrite();
			}
			else if (tmp_tokens[0] == "lighting")
			{
				if (tmp_tokens[1] == "off") pMat->disableLightingEffects();
			}
	...
	Color MaterialManager::_parse_color3(const string & r, const string & g, const string & b)
	{   Real cr, cg, cb;
		cr = _parse_value(r);
		cg = _parse_value(g);
		cb = _parse_value(b);
	...
	Color MaterialManager::_parse_color4(const string & r, const string & g, const string & b, const string & a)
	{   Real cr, cg, cb, ca;
		cr = _parse_value(r);
		cg = _parse_value(g);
		cb = _parse_value(b);
