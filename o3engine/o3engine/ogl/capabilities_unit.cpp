...
const std::vector<image_compressed_format> capabilities_unit::supported_compressed_formats() const {
	...
	for(auto gl_format : gl_formats)
		formats.push_back((image_compressed_format) gl_format);
	return formats;
}
...
bool capabilities_unit::is_program_binary_format_supported(int id) const {
	auto formats = supported_program_binary_formats();
...
void capabilities_unit::max_viewport_dims(int & width, int & height) const {
	auto dims =  m_ctx.get_param_intv(context_param_type::MAX_VIEWPORT_DIMS, 2);
	width = dims[0];
	height = dims[1];
}
