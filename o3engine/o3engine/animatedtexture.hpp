...
namespace o3engine {
	...
	public:
		...
		enum E_ANIMATED_TEXTURES_PLAYMODE {
			...
			PLAY_FORWARDBACK
		//!< Play until the end and then the opposite way until the start
		};
		...
		//! Reset timer to start from the beginning again
		inline void rewind() {
			current_time = 0;
		...
		//! Set playmode
		int setPlayMode(int newmode) {
			return play_mode = newmode;
		}
