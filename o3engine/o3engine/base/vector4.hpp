...
namespace o3engine {
	...
	/**
	 * This class is useful for representing any kind of vector in 4D space.
	 * It is light and generally fast and contains a number of feature
	 * very useful when working with vectors.
	 *
	...
	public:
		...
		inline Vector4 opposite() const {
			return Vector4(-x, -y, -z, -w);
		}
		inline Vector4 operator-() const {
			return Vector4(-x, -y, -z, -w);
		}
		inline Real distance(const Vector4 & r) const {
			return (*this - r).length();
		}
		inline Real squaredDistance(const Vector4 & r) const {
