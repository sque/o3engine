...
namespace o3engine {
	...
	/**
	 * This class is useful for representing any kind of triangle in 3D space.
	...
	public:
		...
		inline Triangle3(const Vector3 & p_a, const Vector3 & p_b,
				const Vector3 & p_c) :
				point_a(p_a), point_b(p_b), point_c(p_c) {
		}

		//! Construct and initialize from an array of 9 Real numbers
		inline Triangle3(const Real coords[]) :
				point_a(coords[0], coords[1], coords[2]), point_b(coords[3],
						coords[4], coords[5]), point_c(coords[6], coords[7],
						coords[8]) {
		}
		...
		inline Real area() const {
			return (point_b - point_a).crossProduct(point_c - point_a).length()
					* 0.5f;
		}
		...
		inline bool operator==(const Triangle3 & r) const {
			return (point_a == r.point_a && point_b == r.point_b
					&& point_c == r.point_c);
		}
