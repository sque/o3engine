...
namespace o3engine {
	...
	/**
	 * This class is useful for representing any kind of vector in 2D space.
	 * It is light and generally fast and contains a number of feature
	 * very useful when working with vectors. It also provides casting
	 * operators to facilitate the interaction of Vector2 objects.
	 *
	...
	public:
		...
		//! Custom constructor
		inline Vector2(Real _x, Real _y) :
				x(_x), y(_y) {
		}
		...
		inline Real distance(const Vector2 & r) const {
			return (*this - r).length();
		}
		inline Real squaredDistance(const Vector2 & r) const {
		...
		//! @{
		inline Vector2 & operator*=(Real factor) {
			x *= factor;
			y *= factor;
			return *this;
		}
		inline Vector2 operator*(Real factor) const {
		...
		inline Vector2 midPoint(const Vector2 & r) const {
			return Vector2((x + r.x) * 0.5, (y + r.y) * 0.5);
		}
		//! @}
