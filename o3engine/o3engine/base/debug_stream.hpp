...
namespace o3engine {
	...
	inline std::string toString(const Color & c) {
		char cStr[255];
		sprintf(cStr, "Color(R:%f G:%f B:%f A:%f)", c.red, c.blue, c.green,
				c.alpha);
		return std::string(cStr);
	}
	...
	inline std::string toString(const Triangle3 & tri) {
		...
		sprintf(cStr, "Tri3((%f, %f, %f) - (%f, %f, %f) - (%f, %f, %f) )",
				tri.point_a.x, tri.point_a.y, tri.point_a.z, tri.point_b.x,
				tri.point_b.y, tri.point_b.z, tri.point_c.x, tri.point_c.y,
				tri.point_c.z);
		return std::string(cStr);
	}
	//! @}
